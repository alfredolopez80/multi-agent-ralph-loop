...
#
# ----------------------------------------------------------------------------
# DIAGNOSTIC 2026-08-25 (T14-runtests2, #51 part 2): WIRED (candidate)
# Verdict on a bare checkout: 6/6 PASS, exit 0. Reads
# /tmp/ralph-statusline-context.json (a cache file that exists whenever the
# statusline hook has run). The 6 assertions check that the cache file
# exists, is valid JSON, has required fields, values are in valid range,
# percentage calculation is correct, and remaining_percentage matches.
# Caveat: assertions pass against any well-formed cache, including the
# zero-default cache that exists before the statusline runs; the test does
# not assert non-zero usage. This is the same "talking-tests" class that
# PR #38 (#42) fixed across 30 other scripts. Out of scope for this task,
# but worth noting if anyone promotes this to a CI gate.
# Verdict: wired (eligible for run-all-unit-tests.sh TEST_SUITES today).
# ----------------------------------------------------------------------------
#
