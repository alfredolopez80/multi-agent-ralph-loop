...
if [[ -n "$MULTI_REPORT" ]]; then
    # After complete_action_report overwrites the report file, the iteration
    # count lives in the markdown table as "| **Iterations** | 3 |". The
    # "Progress Update: Iteration N" markers that mark_iteration wrote are
    # gone — the file is regenerated by generate_markdown_report from the
    # metadata details (action-report-generator.sh:228-230).
    #
    # The grep pipeline is wrapped in `if VAR=$(...); then` so a no-match
    # exit (grep returns 1) does not trip `set -e` + `pipefail` and kill
    # the whole test script. The else branch is the explicit failure path.
    if ITERATIONS=$(grep -oE '\*\*Iterations\*\* \| [0-9]+' "$MULTI_REPORT" 2>/dev/null | grep -oE '[0-9]+' | head -1); then
        ...
        log_fail "Iterations row not found in report (production bug? check action-report-generator.sh)"
    fi
fi
