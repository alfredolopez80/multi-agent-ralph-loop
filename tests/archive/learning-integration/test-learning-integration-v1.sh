...
# This suite asserts against an architecture retired in v3.0:
#   - $HOME/.claude/hooks/learning-gate.sh  (hook archived, no replacement)
#   - $HOME/.claude-sneakpeek/zai/config/settings.json  (sneakpeek never installed)
#   - $HOME/.ralph/curator/scripts/  (curator scripts no longer in user install)
#   - $HOME/.ralph/learning/state.json  (state file not generated by v3.0)
# Tests for the v2.81.2 documentation (docs/implementation/FASE_*.md) DO pass,
# thanks to the REPO_ROOT=BASH_SOURCE fix from #55.
# Verdict: removed (architecture retired, not "needs provisioned machine" only).
# ----------------------------------------------------------------------------
#
