...
| 6 | Trivial task zero-subagent | a NEW probe to be added to the tree (does not exist yet) | does not exist | **PASS = (a) probe exists in `scripts/` or `tests/`, (b) runs on at least 3 trivial task fixtures (single Edit, single Bash echo, single Read), (c) zero subagents are spawned, (d) zero Task/Agent tool calls observed in the trace.** Quantitative target deferred to when the probe is built; the certification does NOT close until this row has a baseline. |
