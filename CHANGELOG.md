...
- **`react-doctor.mjs` reported setup failures as code-quality findings** — the candidate loop accepted the first runner that merely started, so a registry 404, auth, TLS or offline error was written to the output file and presented to the model as "React Doctor found issues in the changed files", while the remaining fallbacks were skipped. A scan reports on stdout; a runner that never executed the tool does not. Same class, second instance: the hook is registered globally, so in a repo with no JavaScript react-doctor exits non-zero with "No React project found" — now skipped when there is no root `package.json`
