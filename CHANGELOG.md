...
- **An unreachable subagent-stop guard certified as working** — `ralph-subagent-stop.sh` Check 1 blocks on subagent status `"working"`, but that vocabulary belongs to the Agent Teams teammate files the runtime owns; this check reads repo-owned state whose only producer, `ralph-subagent-start.sh`, writes `"active"`, so the branch never fires. The integration suite hid it by fabricating `{"status":"working"}`. The test now uses the vocabulary production writes and exercises the live guard (assigned task status), plus a pin asserting `"active"` never blocks — broadening the status would deadlock every subagent stop, since nothing transitions it before `SubagentStop` and the hook has no bounded-block valve
