...
- **An unreachable subagent-stop guard certified as working** — `ralph-subagent-stop.sh` Check 1 blocked on subagent status `"working"`, but that vocabulary belongs to the Agent Teams teammate files the runtime owns; this check read repo-owned state whose only two writers produce `"active"` (`ralph-subagent-start.sh`) and `"completed"` (this hook), so the branch could never fire. The integration suite hid it by fabricating `{"status":"working"}`. The dead branch is removed rather than repaired: every way to make it fire either duplicates the assigned-task check or blocks unconditionally, since nothing transitions the status before `SubagentStop` and the hook has no bounded-block valve. Unfinished work is detected by the assigned-task status, which the test now drives with real vocabulary, plus a pin asserting `"active"` never blocks
- **`react-doctor.mjs` registered but untracked** — `settings.json` runs it globally on `PostToolBatch` via `$CLAUDE_PROJECT_DIR`, yet the file existed in only one worktree, so every other checkout resolved the path to nothing and `node` failed on each tool batch. Now tracked
- **The hang/block probe only globbed `*.sh`** — every non-shell hook registered in `settings.json` escaped it. Extended to `.mjs` and `.py`
- **`react-doctor.mjs` reported setup failures as code-quality findings** — the candidate loop accepted the first runner that merely started, so a registry 404, auth, TLS or offline error was presented to the model as "React Doctor found issues in the changed files", while the remaining fallbacks were skipped. Same class, second instance: the hook is registered globally, so in a repo with no JavaScript react-doctor exits non-zero with "No React project found" — also reported as findings

### Changed - react-doctor hook reads the structured report

The hook now runs react-doctor with `--json --json-compact` and ignores the exit code entirely. react-doctor exits non-zero both when it finds problems and when it cannot run at all, and the package managers behind it exit non-zero for registry, auth, TLS and offline errors, so the code cannot distinguish them; the report can — `error` carries setup failures, `diagnostics` carries findings. `--json` also suppresses all other output, so stderr can no longer contaminate what is reported. The whole class of false "found issues" warnings is now excluded by construction rather than by heuristic.

Consequences: only `error` severity and `Security` category diagnostics interrupt the loop, since reporting every warning on every edit batch is noise; output is capped at 20 diagnostics with errors first and formatted from `rule`, `filePath:line:column`, `message` and `help` instead of dumping unbounded `--verbose` text; an unrecognised `schemaVersion` is treated as "cannot read this" and stays silent. A cheap root `package.json` check still short-circuits non-JS repos before any network fetch.
- **The hang/block probe judged hooks on an event they never receive** — the deny verdict was applied to all 78 hooks, including ones the runtime only hands `SessionStart` or `Stop` payloads to; five of them can legitimately emit `{"decision": "block"}` for their own event, so the suite was green by luck. The verdict is now scoped to the 15 hooks that speak the `PreToolUse` dialect, while terminating and single-object output still apply to all 78
