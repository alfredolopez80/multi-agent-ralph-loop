...
`export QTEAM_TEST_CMD="bash tests/run-all-unit-tests.sh"` — runner asserts `failed == 0` AND `total > 0`, and runs the full `pytest tests/` sweep, a superset of CI's "Run Tests" job.
