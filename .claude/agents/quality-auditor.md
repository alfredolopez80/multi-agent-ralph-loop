...
## Autoresearch Checks Template (v2.95)

Quality Auditor can generate a project-specific `autoresearch.checks.sh` script that autoresearch uses as its validation command. The script is auto-detected based on project type and outputs minimal, machine-parseable results.

### Project Type Detection and Templates

| Project Type | Detection Signal | Check Command |
|--------------|------------------|---------------|
| TypeScript | `tsconfig.json` present | `pnpm typecheck && pnpm test --run` |
| Python | `pyproject.toml` or `setup.py` present | `python -m mypy src/ && pytest -q` |
| Rust | `Cargo.toml` present | `cargo clippy && cargo test` |

### Generated Script Template

```bash
#!/usr/bin/env bash
# autoresearch.checks.sh — generated by quality-auditor
# Usage: ./autoresearch.checks.sh
...
# --- Python template ---
# python -m mypy src/ --no-error-summary 2>&1 | tail -1
# pytest -q --tb=line 2>&1 | tail -3

# --- Rust template ---
# cargo clippy --message-format=short 2>&1 | grep "^error" | wc -l
# cargo test 2>&1 | tail -3

# --- Secondary metrics extraction ---
# These lines are parsed by autoresearch for tracking

type_errors=$(pnpm typecheck 2>&1 | grep -c "error TS" || echo 0)
echo "METRIC type_errors=$type_errors"

test_output=$(pnpm test --run --coverage 2>&1)
test_coverage=$(echo "$test_output" | grep "All files" | awk '{print $10}' || echo "0%")
echo "METRIC test_coverage=$test_coverage"

lint_warnings=$(pnpm lint 2>&1 | grep -c "warning" || echo 0)
echo "METRIC lint_warnings=$lint_warnings"

# Final pass/fail based on primary check
if [ "$type_errors" -eq 0 ]; then
  echo "METRIC result=pass"
  exit 0
else
  echo "METRIC result=fail"
  exit 1
fi
```
...
|--------|--------|---------|
| `type_errors=N` | Integer count | Track type safety improvements |
| `test_coverage=NN%` | Percentage | Track test coverage trend |
| `lint_warnings=N` | Integer count | Track code quality warnings |

### Design Principles

- **Minimal output**: Suppress success messages, show only errors
- **Machine-parseable**: All metrics use `METRIC key=value` format
- **Fast execution**: Skip slow checks (e2e tests, full builds) unless explicitly requested
- **Deterministic**: No network calls, no external service dependencies
- **Exit code contract**: 0 = all checks pass, non-zero = at least one failure
